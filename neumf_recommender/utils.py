...
def read_data_ml100k(data_dir:str)->tuple[pd.DataFrame,int,int]:
    """
    Reads the MovieLens 100K dataset from the specified directory and returns the data along with the number of unique users and items.

    Parameters:
    -----------
    data_dir : str
        The directory where the MovieLens 100K dataset file ('u.data') is located.

    Returns:
    --------
    tuple[pd.DataFrame, int, int]
        A tuple containing:
        - data (pd.DataFrame): The dataset containing user-item interactions with columns 'user_id', 'item_id', 'rating', and 'timestamp'.
